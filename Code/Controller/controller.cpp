...
float[3] Controller::FrontDetect(float sonar, float targetDistance){
	float[3] out;
...
float[3] Controller::ApplyPID(float[3] in, float timeStep){
	float[3] out;
	// Apply the PID control to the inputs
	out[0] = this->PIDVx.GetControl(in[0], timeStep, this->isSaturated[0]);
	out[0] = this->PIDVy.GetControl(in[1], timeStep, this->isSaturated[1]);
	out[0] = this->PIDW.GetControl(in[2], timeStep, this->isSaturated[2]);
